...
class Orquestador:
    ...
    async def iniciar(self) -> None:
        ...
        # El callback ahora recibe el audio directamente del VAD
        self._audio = ModuloAudio(
            callback_voz=self._on_voz,
            loop=self._loop,
        )
    ...
    # ─────────────────────────────────────────────
    #  Callback del VAD — recibe audio ya grabado
    # ─────────────────────────────────────────────

    async def _on_voz(self, audio_np: np.ndarray) -> None:
        """
        Llamado por VADDetector cuando detecta una frase larga.
        El audio ya está capturado — solo hay que transcribir y procesar.
        """
        ...
        try:
            ...
            log.exception("Error en pipeline de voz: %s", e)
        finally:
            self._procesando = False
    ...
    async def _pipeline(self, texto_usuario: str) -> str:
        ...
        await self._broadcaster.broadcast(
            {"tipo": "expresion", "emocion": estado["emocion"]}
        )
        ...
        try:
            ...
            msg = "Tuve un problema al generar la respuesta."
