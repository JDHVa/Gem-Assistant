...
class MemoriaRAG:
    ...
    async def guardar(
        ...
        try:
            ...
            log.warning("No se pudo generar embedding (%s). Se omite guardado.", e)
            return  # Salir sin guardar, sin lanzar excepción
        meta = {"timestamp": datetime.now().isoformat(), "coleccion": coleccion}
        if metadata:
            meta.update(metadata)
        self._cols[coleccion].add(
            ids=[str(uuid.uuid4())],
    ...
    # En guardar_comando_exitoso, también capturar error:
    async def guardar_comando_exitoso(self, instruccion: str, comando: str) -> None:
        try:
            await self.guardar(
                f"Instrucción: {instruccion}\nComando: {comando}",
            ...
            log.warning("No se pudo guardar comando exitoso: %s", e)
