...
class Ajustes(BaseSettings):
    ...
    gemini_modelo_tts: str = "gemini-2.5-flash-tts"
    ...
    usar_openwakeword: bool = True
    wake_word_modelo: str = (
        "hey jarvis"  # modelos pre-entrenados: "alexa", "hey jarvis", "hey mycroft", etc.
    )
    wake_word_umbral: float = 0.5

    # ───── Wake word: fallback RMS (sin openWakeWord) ─────
    fallback_rms_umbral: float = 0.15  # más alto = menos falsos positivos
    fallback_activacion_s: float = 1.2  # tiempo de voz sostenida
    fallback_cooldown_s: float = (
        5.0  # tras activar, ignorar (evita auto-trigger por TTS)
    )
    ...
    # ───── Avatar (PNGs) ─────
    # Ruta donde están las carpetas de emociones con los PNGs
